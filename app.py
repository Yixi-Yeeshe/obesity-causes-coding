...
# New app settings
# =============================
RAW_SHEET = "obesity_causes"
KAPPA_SHEET = "causes_kappa"

DATA_PATH = "obesity causes.csv"

OPTIONS = [
    "1. obesity as inevitable",
    "2. obesity as self-inflicted",
...
st.set_page_config(page_title="Obesity Causes Coding", layout="wide")
st.title("Obesity Causes Coding Task")
...
def update_kappa_format(kappa_ws, df):
    """
    Rebuild causes_kappa from obesity_causes.
...
st.write("obesity_causes = 原始长表，每一行是一个 coder 对一个 S 片段的判断。")
st.write("causes_kappa = 可用于计算 kappa 的宽表。")

if st.button("手动更新 causes_kappa 表"):
    latest_df = read_raw_data_cached()
    update_kappa_format(kappa_ws, latest_df)
    st.success("causes_kappa 已更新。")
